...
def main(args: argparse.Namespace):
    ...
    requests1 = requests[:len(requests)//2]
    requests2 = requests[len(requests)//2:]

    # 在main函数中
    with concurrent.futures.ProcessPoolExecutor() as executor:
        # 分别为两个进程设置不同的GPU
        future1 = executor.submit(run_vllm_on_specific_gpus, requests1, args.model, tokenizer, 4, [0, 1, 2, 3])
        future2 = executor.submit(run_vllm_on_specific_gpus, requests2, args.model, tokenizer, 4, [4, 5, 6, 7])
        
        # 等待任务完成并获取结果
        elapsed_time1, input_num_tokens1, output_num_tokens1 = future1.result()
        elapsed_time2, input_num_tokens2, output_num_tokens2 = future2.result()

    total_elapsed_time = max(elapsed_time1,elapsed_time2)
    total_input_num_tokens = input_num_tokens1 + input_num_tokens2
    total_output_num_tokens = output_num_tokens1 + output_num_tokens2
